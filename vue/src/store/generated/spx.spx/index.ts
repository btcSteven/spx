import { Client, registry, MissingWalletError } from 'spx-client-ts'

import { Params } from "spx-client-ts/spx.spx/types"
...
export default {
	...
	actions: {
		init({ dispatch, rootGetters }) {
			console.log('Vuex module: spx.spx initialized!')
		...
		async QueryParams({ commit, rootGetters, getters }, { options: { subscribe, all} = { subscribe:false, all:false}, params, query=null }) {
			try {
				...
				let value= (await client.SpxSpx.query.queryParams()).data
		...
		async QueryHello({ commit, rootGetters, getters }, { options: { subscribe, all} = { subscribe:false, all:false}, params, query=null }) {
			try {
				const key = params ?? {};
				const client = initClient(rootGetters);
				let value= (await client.SpxSpx.query.queryHello()).data
